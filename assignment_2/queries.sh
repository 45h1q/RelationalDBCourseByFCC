...
echo "$($PSQL "SELECT DISTINCT(winner.name) FROM games INNER JOIN teams AS winner ON games.winner_id = winner.team_id  WHERE year=2014 AND round='Eighth-Final' UNION SELECT DISTINCT(opponent.name) FROM games INNER JOIN teams as opponent ON games.opponent_id = opponent.team_id WHERE year=2014 AND round='Eighth-Final' ORDER BY name  ")"
